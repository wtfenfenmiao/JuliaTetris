    ...
    newInstance: function(blockType,blockSize) {
        ...
        switch (blockType) {
            ...
            case 7:
                ...
                break;
            //此处加个戒指。如果加点击按钮太麻烦，就戒指到了之后，直接出信，弹出页面那种。一点点做。
            case 8:
                block = new Ring(blockSize);
                break;
        }
        return block;
    }
...
function Ring(blockSize) {
    ...
    if (typeof Ring.prototype.draw !== "function") {
        Ring.prototype.draw = function(ctx) {
            ...
            //ctx.stroke();
            ctx.drawImage(img, this.x-this.blockSize, this.y-this.blockSize, 2*this.blockSize, 2*this.blockSize);
            ctx.closePath();
            
            this.calMatrix();
        }
        Ring.prototype.calMatrix = function() {
            //使用点阵系统便于计算
